...
setUser(){
	...
	echo "\033[32m 注意： \033[0mchacha20 需要安装 libsodium 支持库，否则会启动失败！"
	echo
	...
	if [[ $? -eq 0 ]]; then
		if [[ ${ssspeed_limit_per_con} -ge 1 ]] && [[ ${ssspeed_limit_per_con} -le 99999 ]]; then
			echo
			echo "——————————————————————————————"
			echo -e "	单端口单线程 : \033[41;37m ${ssspeed_limit_per_con} KB/S \033[0m"
			echo "——————————————————————————————"
			echo
			break
		else
	...
	if [[ $? -eq 0 ]]; then
		if [[ ${ssspeed_limit_per_user} -ge 1 ]] && [[ ${ssspeed_limit_per_user} -le 99999 ]]; then
			echo
			echo "——————————————————————————————"
			echo -e "	单端口总限速 : \033[41;37m ${ssspeed_limit_per_user} KB/S \033[0m"
			echo "——————————————————————————————"
			echo
			break
		else
	...
	echo -e " 端口\t    : \033[41;37m ${ssport} \033[0m"
	echo -e " 密码\t    : \033[41;37m ${sspwd} \033[0m"
	echo -e " 加密\t    : \033[41;37m ${ssmethod} \033[0m"
	echo -e " 协议\t    : \033[41;37m ${ssprotocol} \033[0m"
	echo -e " 混淆\t    : \033[41;37m ${ssobfs} \033[0m"
	echo -e " 设备数限制 : \033[41;37m ${ssprotocol_param} \033[0m"
	echo -e " 单线程限速 : \033[41;37m ${ssspeed_limit_per_con} KB/S \033[0m"
	echo -e " 端口总限速 : \033[41;37m ${ssspeed_limit_per_user} KB/S \033[0m"
	echo "——————————————————————————————"
	echo
...
installSSR(){
	...
	if [[ -z ${JQ_ver} ]]; then
		...
		echo -e "\033[42;37m [信息] \033[0m JSON解析器 JQ 已安装，继续安装ShadowsocksR..."
	fi
	cd /etc
	...
	if [[ ${ssprotocol} != "origin" ]]; then
		...
		if [[ $? -eq 0 ]]; then
			if [[ ${ssprotocol_param} -ge 1 ]] && [[ ${ssprotocol_param} -le 99999 ]]; then
				echo
				echo "——————————————————————————————"
				echo -e "	设备数 : \033[41;37m ${ssprotocol_param} \033[0m"
				echo "——————————————————————————————"
				echo
				break
			else
