...
check_deb_off(){
	get_latest_new_version
	deb_ver=`dpkg -l|grep linux-image | awk '{print $2}' | awk -F '-' '{print $3}' | grep '4.9'`
	if [[ "${deb_ver}" != "" ]]; then
		...
		echo -e "\033[41;37m [错误] \033[0m 检测到 内核版本 不支持开启BBR，请使用 bash bbr.sh install 来安装 !" && exit 1
	fi
}
# 删除其余内核
del_deb(){
...
#修改DNS为8.8.8.8
	...
	wget -N -O ${deb_kernel_name} "${deb_kernel_url}"
