...
Download_ocserv(){
	...
	tar -xJf ocserv-0.12.4.tar.xz && cd ocserv-0.12.4
	./configure
	make
	make install
	cd .. && cd ..
	...
	if [[ -e ${file} ]]; then
		mkdir "${conf_file}"
		wget --no-check-certificate -N -P "${conf_file}" "https://raw.githubusercontent.com/Androad/doubi/master/other/ocserv.conf"
